...
from arxiv.db.models import DBLaTeXMLDocuments, DBLaTeXMLSubmissions, Metadata, Submission
...
def get_license_for_submission(submission_id: int) -> str:
    license_raw = session.execute(select(Submission.license).filter(Submission.submission_id == submission_id)).scalar()
    return license_url_to_str_mapping(license_raw)
...
def _write_start_sub(submission_id: int, checksum: str) -> None:
    with transaction() as session:
        rec = session.query(DBLaTeXMLSubmissions).filter(DBLaTeXMLSubmissions.submission_id == submission_id).first()
        if not rec:
...
def _write_success_sub(submission_id: int, checksum: str) -> None:
    with transaction() as session:
        obj = session.query(DBLaTeXMLSubmissions).filter(int(submission_id) == DBLaTeXMLSubmissions.submission_id).one()
        obj = obj
        if (
            obj.tex_checksum == checksum
            and obj.latexml_version == current_app.config["LATEXML_COMMIT"]
